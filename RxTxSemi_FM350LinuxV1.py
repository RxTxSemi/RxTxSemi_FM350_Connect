...
# Helper for PyInstaller to load data files both in dev and bundled app
...
class FibocomMonitor:
    ...
    def setup_ui(self):
        ...
        try:
            logo_img = Image.open(resource_path('rxtxsemi_logo.png'))
        ...
        try:
            qr_img = Image.open(resource_path('bmc_qr.png'))
    ...
    def connect_to_modem(self):
        ...
        try:
            ...
            # Ensure the interface is UP before nmcli
            self.update_status(f"Bringing up interface {interface}...")
            print(f"[DEBUG] Using interface: {interface}")
            result = self.run_privileged_command(f"ip link set {interface} up")
            if result.returncode != 0:
                messagebox.showerror("Network Error", f"Failed to bring up interface {interface}: {result.stderr}")
                return

            # Step 7: Use nmcli to configure the modem network interface
            self.update_status("Configuring network via NetworkManager (nmcli)...")

            # Clean up old connection if exists
            self.run_privileged_command(f"nmcli connection delete 'RxTxSemi-FM350' || true")

            # Create new manual connection with static IP
            gateway_ip = '.'.join(ip_address.split('.')[:-1]) + '.1'
            nmcli_cmd = (
                f"nmcli connection add type ethernet con-name 'RxTxSemi-FM350' ifname {interface} "
                f"ipv4.method manual ipv4.addresses {ip_address}/24 ipv4.gateway {gateway_ip} "
                f"ipv4.dns '8.8.8.8,1.1.1.1' autoconnect yes"
            )
            result = self.run_privileged_command(nmcli_cmd)
            if result.returncode != 0:
                messagebox.showerror("Network Error", f"Failed to configure network via nmcli: {result.stderr}")
                return

            # Bring the connection up
            result = self.run_privileged_command(f"nmcli connection up 'RxTxSemi-FM350'")
            if result.returncode != 0:
                messagebox.showerror("Network Error", f"Failed to bring up network via nmcli: {result.stderr}")
                return
    ...
    def find_modem_interface(self):
        """Find the modem network interface in a robust way."""
        try:
            # Get all network interfaces except lo and docker0
            interfaces = [iface for iface in os.listdir('/sys/class/net')
                          if iface not in ('lo', 'docker0') and
                          (iface.startswith('enx') or iface.startswith('wwan'))]
            print(f"[DEBUG] Found candidate interfaces: {interfaces}")
            ...
            for iface in interfaces:
                ...
                try:
                    with open(state_path) as f:
                        state = f.read().strip()
                    print(f"[DEBUG] Interface {iface} state: {state}")
                    if state in ('up', 'unknown'):
                        candidates.append(iface)
                except Exception as e:
                    print(f"[DEBUG] Could not read state for {iface}: {e}")
                    continue

            # If no UP interfaces, fall back to all candidates
            if not candidates:
                candidates = interfaces
                print("[DEBUG] No interfaces are UP, falling back to all candidates.")

            # Print MAC addresses for all candidates
            for iface in candidates:
                try:
                    mac = open(f'/sys/class/net/{iface}/address').read().strip()
                    print(f"[DEBUG] Interface {iface} MAC: {mac}")
                except Exception as e:
                    print(f"[DEBUG] Could not read MAC for {iface}: {e}")

            # Return the first candidate found
            if candidates:
                print(f"[INFO] Selected modem interface: {candidates[0]}")
                return candidates[0]

            print("[ERROR] No suitable modem interface found.")
            return None
        except Exception as e:
            print(f"[ERROR] Exception in find_modem_interface: {e}")
            return None
    ...
    def disconnect_from_modem(self):
        """Disconnect from the modem and clean up network configuration"""
        try:
            self.update_status("Disconnecting from modem...")
            # Stop monitoring
            self.monitoring = False
            ...
            # Clean up nmcli connection
            self.run_privileged_command("nmcli connection delete 'RxTxSemi-FM350' || true")
            if self.interface_name:
                self.run_privileged_command(f"nmcli device disconnect {self.interface_name} || true")
                self.run_privileged_command(f"ip link set {self.interface_name} down || true")
            ...
            self.status_label.config(text="Disconnected", style='Warning.TLabel')
            self.update_status("Disconnected from modem.")
        except Exception as e:
            messagebox.showerror("Disconnect Error", f"Failed to disconnect from modem: {str(e)}")
            self.update_status("Disconnection failed")
    ...
    def show_about(self):
        ...
        # Buy Modem button (added to match Windows)
        def open_buy():
            webbrowser.open('http://rxtxsemi.com/Fibocom-fm350gl')
        ttk.Button(frame, text="Buy FM350-GL", command=open_buy).pack(pady=(10, 0))
        # Close button
...
def main():
    ...
    # Load and set app icon (for dock/taskbar)
    icon_path = resource_path("icon.png")  # or your installed icon
